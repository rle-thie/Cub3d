...
/*   Updated: 2022/12/02 22:16:19 by rle-thie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	get_texture_data(t_data *data, int n)
{
	if (data->raytracing->side == 0)
		data->raytracing->wall_x = data->player->y + data->raytracing->pwd * data->raytracing->dir_y;
	else
		data->raytracing->wall_x = data->player->x + data->raytracing->pwd * data->raytracing->dir_x;
	data->raytracing->wall_x -= floor((data->raytracing->wall_x));
	data->raytracing->text_x = (int)(data->raytracing->wall_x * (float)data->texture[n].w);
	if (data->raytracing->side == 0 && data->raytracing->dir_x > 0)
		data->raytracing->text_x = data->texture[n].w - data->raytracing->text_x - 1;
	if (data->raytracing->side == 1 && data->raytracing->dir_y < 0)
		data->raytracing->text_x = data->texture[n].w - data->raytracing->text_x - 1;
	data->raytracing->step = 1.0 * (float)data->texture[n].h / data->raytracing->line_h;
	data->raytracing->text_pos = (data->raytracing->draw_start - data->height / 2 + data->raytracing->line_h / 2) * data->raytracing->step;
}
...
void	draw_texture(t_data *data, int x)
{
	...
	{
		data->raytracing->text_y = (int)data->raytracing->text_pos & (data->texture[data->raytracing->text_num].h - 1);
		data->raytracing->text_pos += data->raytracing->step;
		if (y < data->height && x < data->width)
		{
			data->mlx->addr[y * data->width + x] = data->texture[data->raytracing->text_num].addr[data->raytracing->text_y * data->texture[data->raytracing->text_num].w + data->raytracing->text_x];
		}
	}
}